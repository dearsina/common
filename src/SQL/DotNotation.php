	...
	public static function normalise3($array): ?array
	{
		...
		foreach($array as $single_row){
			...
			if($row_number){
                ...
                $jsonString = json_encode($scalar[$row_number]);
                $other_row_number = array_search($jsonString, $scalar_row_strings);
                
                if ($other_row_number !== false) {
			...
			# Store all scalar values4 as strings so that they can be matched even when rows are not next to each other
...
//               $scalar_row_strings[$row_number] = implode("", $scalar[$row_number]);
                ...
                $scalar_row_strings[$row_number] = $jsonString;
            }
